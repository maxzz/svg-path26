...
const ensureHistoryReadyAtom = atom(null, (get, set) => {
...
const pushHistoryAtom = atom(null, (get, set, nextPath: string) => {
...
const setPathWithoutHistoryAtom = atom(null, (_get, set, nextPath: string) => {
...
export const parseStateAtom = atom<ParseState>((get) => {
...
export const commandRowsAtom = atom<SvgSegmentSummary[]>((get) => {
    const model = get(parseStateAtom).model
    return model ? model.getSummaries() : []
})

export const targetPointsAtom = atom((get) => get(commandRowsAtom).map((row) => row.target))

export const canvasViewBoxAtom = atom<[number, number, number, number]>((get) => {
    const model = get(parseStateAtom).model
...
const applyModelAtom = atom(null, (get, set, updater: (svg: SvgPathModel) => void) => {
    const path = get(rawPathAtom).trim()
    if (!path) return
...
export const doNormalizePathAtom = atom(null, (get, set) => {
    set(applyModelAtom, () => {})
})

export const doSetMinifyAtom = atom(null, (_get, set, minify: boolean) => {
    set(minifyOutputAtom, minify)
    set(doNormalizePathAtom)
})

export const doSetRelativeAtom = atom(null, (_get, set) => {
    set(applyModelAtom, (model) => model.setRelative(true))
})

export const doSetAbsoluteAtom = atom(null, (_get, set) => {
    set(applyModelAtom, (model) => model.setRelative(false))
})

export const doApplyScaleAtom = atom(null, (get, set) => {
...
export const doApplyTranslateAtom = atom(null, (get, set) => {
...
export const doClearPathAtom = atom(null, (_get, set) => {
